...
# 命令：
# python power_forecasting_itransformer.py train

# 可选参数示例：
# .\.venv\Scripts\python.exe .\power_forecasting_itransformer.py train --epochs 12 --batch-size 1024 --early-stopping-patience 4
...
# 命令：
# python power_forecasting_itransformer.py export

# 可选参数示例：
# python power_forecasting_itransformer.py export --data-root ./data --output-dir ./submission --start-time "2025-11-01 00:00:00"
...
class ITransformerModel(nn.Module):
    def __init__(
        ...
        self.history_steps = history_steps
        self.horizon_steps = horizon_steps
        self.token_projection = nn.Linear(history_steps, hidden_dim)
        ...
        self.head = nn.Sequential(
            nn.Linear(hidden_dim * 2 + 16 + 16, hidden_dim),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim, horizon_steps),
        )
    ...
    def forward(self, x: torch.Tensor, station_idx: torch.Tensor) -> torch.Tensor:
        # iTransformer: treat each variable as a token and its lookback window as token content.
        token_inputs = x.transpose(1, 2)
        token_embeddings = self.token_projection(token_inputs)
        token_embeddings = self.token_norm(token_embeddings)
        encoded = self.encoder(token_embeddings)
        summary = encoded.mean(dim=1)
        target_token = encoded[:, -1, :]
        station_vec = self.station_embedding(station_idx)
        power_history = x[:, :, -1]
        recent_1h = power_history[:, -4:].mean(dim=1, keepdim=True)
        recent_4h = power_history[:, -16:].mean(dim=1, keepdim=True)
        ...
        latest_power = power_history[:, -1:].contiguous()
        history_stats = self.history_projection(torch.cat([latest_power, recent_1h, recent_4h, recent_24h], dim=1))
        fused = torch.cat([summary, target_token, station_vec, history_stats], dim=1)
        return self.head(fused)

DirectMultiHorizonModel = ITransformerModel
...
def load_artifact(path: Path, device: torch.device) -> Dict[str, object]:
    ...
    model_type = payload.get("model_type", "gru")
    model_cls = ITransformerModel if model_type == "itransformer" else LegacyGRUModel
    model = model_cls(
